...
public class Account {
	...
	private final String accountNo; // unique for every account (it's const because it can't be changable)
	...
	protected double withdrawLimit_perDay;
	private LocalDate creatingDate;
	protected LocalDate creditCardDate;
	protected LocalDate debitDate;
	private String accountType = null;

	public Account(String accountType, double limit) {
		userName = null;
		password = null;
		clientName = null;
		balance = 0;
		indebtedness = 0;
		withdrawLimit_perDay = limit;
		status = false;
	...
	public void setAllowableLimit(double aLimit) {
		withdrawLimit_perDay = aLimit > 0 ? aLimit : 0;
	}

	public double getAllowableLimit() {
		return withdrawLimit_perDay;
	}
	...
	public void pay(Frame frame) {
		if (status) {
			...
			if (money == (indebtedness + 0.1 * indebtedness)) {
				...
				JOptionPane.showMessageDialog(frame, "you must pay all your debts");
			}
		} else {
			JOptionPane.showMessageDialog(frame, "You aren't in debt ");
		}
	}
