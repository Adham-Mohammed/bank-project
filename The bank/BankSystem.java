...
public class BankSystem {
	...
	public void setCNum(int currNum) {
	...
	public boolean addAccount(String cName, String uName, String password, Frame frame) {
		...
		while (true) {
			if (cName.isEmpty() || uName.isEmpty() || password.isEmpty()) {
				JOptionPane.showMessageDialog(frame, "Please Enter all the data ");
				check = true;
				break;
			}
			check = false;
			...
			if (check) {
				JOptionPane.showMessageDialog(frame, "Sorry it's arleady used ");
				break;
			} else {
			...
			if (!check) {
				...
				if (!(accounts[index].getPassword().equals(password))) {
					...
					if (accounts[index].isBlocked()) {
						JOptionPane.showMessageDialog(frame, "Sorry, go to the bank to reset your account");
						return false;
					}
					break;
				}
	...
	public void searchForAccount(Frame frame) { // we should add exceptions here
		...
		String id = JOptionPane.showInputDialog(frame, "Enter account no ");
		boolean check = false;
		if (id != null) {
			for (int i = 0; i < currNum; i++) {
				if (id.equals(accounts[i].getaccountNo()) && !id.equals(accounts[index].getaccountNo())) {
					check = true;
			...
			if (!check) {
				JOptionPane.showMessageDialog(frame, "invalid account no ");
			}
		}
	}
}