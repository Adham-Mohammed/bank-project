...
public class Interface {
	...
	Color framesColor = new Color(5, 26, 6);
	BankSystem bankSystem = new BankSystem();

	public Interface() {
		Frame frame1 = new Frame("interface");
		JButton button1 = new JButton("login");
		JButton button2 = new JButton("Sign up");
		ImageIcon mainIcon = new ImageIcon("photo/logo.png");
				...
				ActionListener al5 = new ActionListener() {
					public void actionPerformed(ActionEvent ae) {
						if (bankSystem.openAccount(username.getText(), password.getText(), frame3)) {
							frame3.dispose();
							new ChooseAction(bankSystem);
						}
					}
				};

				ImageIcon loginIcon = new ImageIcon("photo/logo.png");
		...
		ActionListener al2 = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ImageIcon signUpIcon = new ImageIcon("photo/logo.png");
	...
	public double moneyExceptions(Frame frame) {
		...
		while (true)
			try {
				...
				double money = Double
						.parseDouble(JOptionPane.showInputDialog(frame, "please enter the limit: "+ leastMoney+" $ or more"));
				if (money >= leastMoney) {
					return money;
				} else
					JOptionPane.showMessageDialog(frame, "please enter the limit: "+ leastMoney+" $ or more" );
			} catch (Exception ex) {
